...
With {pn_RKE1}, the operation of Kubernetes is easily automated and entirely independent of the operating system and platform you’re running. Using a supported version of the container runtime engine, you can deploy and run Kubernetes with {pn_RKE1}. It builds a cluster from a single command in just a few minutes, and its declarative configuration makes Kubernetes upgrades atomic and safe.
