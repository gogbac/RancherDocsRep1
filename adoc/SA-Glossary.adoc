...
A guide with the basic steps to deploy the layered stack of components from both the {companyName} and partner porfolios. This is considered a fundamental basis to demonstrate a specific, tested configuration of components.
+
endif::RC[]
ifdef::RA[]
...
A guide with the general steps to deploy and validate the structured solution components from both the {companyName} and partner porfolios. This provides a shareable template of consistency for consumers to leverage.
+
endif::RA[]
// ifdef::SS[]
// [[G_Solution-Stack]]Solution Stack::
// A guide with the basic steps to deploy the highlighted components of the {companyName} portfolio, with generalized pointers to other elements. This is considered a fundamental approach and a basis for many other tested and supported variations.
// endif::SS[]
//+
// ifdef::EA[]
