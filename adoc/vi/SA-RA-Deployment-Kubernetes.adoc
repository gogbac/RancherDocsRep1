...
NOTE: This process is applicable for deploying a {pn_K3s} cluster for the {pn_Rancher} server or as a downstream cluster managed by {pn_Rancher}. In the case of a downstream cluster, import the {pn_K3s} cluster according to the instructions in xref:/SA-RA-Deployment-Rancher.adoc[ImportCluster]
