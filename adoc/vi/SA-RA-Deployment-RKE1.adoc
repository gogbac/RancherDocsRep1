...
* <<G_Availability>>
** While a single {pn_RKE1} node works perfectly fine, a full high-availability {pn_RKE1} cluster is recommended for production workloads. For this use case, two additional hosts should be added; for a total of three. All three hosts will perform the roles of control-plane, etcd, and worker.
