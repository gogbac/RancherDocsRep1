...
. Log into the first server node and create a new config.yaml file for the remaining two nodes:
...
RKE2_VERSION=""          # Match the first of the first server (Hint: `kubectl get nodes`)
----

cp -p /etc/rancher/rke2/config.yaml .
