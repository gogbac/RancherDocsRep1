...
For this Proof of Concept deployment, a single server installed with the SLE-Micro 5.0 immutable operating system will support a single instance of K3s. For maximum flexibility, K3s will be deployed in a manner that would allow expanding the single-node cluster into a highly available, three-node Kubernetes cluster. This will be only slightly more complex that the one-line, 45 second (curl -sfL https://get.k3s.io | sh -) minimal installation method of K3s. 
...
==== Rancher Server Software deployment - Sub-Section 6.3.4

Rancher server is a complete solution for managing Kubernetes clusters and Kubenetes applications. It addresses the operational and security challenges of managing multiple Kubernetes clusters and applications across any infrastructure. Rancher streamlines Kubernetes cluster management on bare metal servers, private clouds, and vSphere environements; from the datacenter to the edge. Rancher unites all of your Kubernetes clusters with global security policies, centralized authentication, access control and observability.

As Rancher server is a native Kubernetes application, it will run on the single-node K3s cluster. In instances where a load balancer is used to support the K3s cluster, deploying two additional K3s cluster nodes will automatically make Rancher server highly available. Rancher server uses the K3s etcd key/value store to persist its data, which offers several advantages. Additional, highly available, storage isn't needed to make Rancher server highly available. In addition, backing up the K3s etcd store protects the cluster as well as the installation of Rancher server. 
